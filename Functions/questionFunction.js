...
async function getUserScore(user_id, batch_id) {
	...
	try {
		...
		for (i = 0; i < rows.length; i++) {
			if (
				rows[i].option_answer ==
				row.rows[i].user_answer
			) {
				score = score + 1;
			}
		}
...
async function getQuestion(batch_id) {
	...
	try {
		const { rows } = await db.query(queryObj);
		return Promise.resolve(shuffle(rows));
	} catch (e) {
		console.log(e);
		return Promise.reject({
			status: "error",
	...
	try {
		...
		if (rowCount > 0) {
			return Promise.resolve({
				...
				code: 200,
				message:
					"Applicant score updated successfully",
			});
		}
	} catch (e) {
		console.log(e);
	...
	try {
		...
		if (rowCount == 0) {
			return Promise.reject({
				...
				code: 400,
				message:
					"Error creating Assessment History",
			});
		}
		if (rowCount > 0) {
			return Promise.resolve({
				...
				code: 201,
				message:
					"Assessment History created sucessfully",
			});
		}
	} catch (e) {
		console.log(e);
	...
	try {
		...
		if (rowCount > 0) {
			return Promise.reject({
				...
				code: "400",
				message:
					"Assessment History Already Submitted",
			});
		}
	} catch (e) {
		console.log(e);
